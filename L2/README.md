...
что тогда? Надеюсь вы уже поняли к чему я веду. Нам надо сделать так, чтобы возраст мы меняли только один раз и оно сразу менялось везде.
Для этого можно использовать _переменные (variables)_.
...
* Для того, чтобы машина поняла, что внутри текст находится переменная, ее нужно особым образом пометить. Для этого мы заключаем
  переменную `age` в конструкцию `${ }`

Теперь иы опять обрели полный контроль над нашей программой!
...
Я не упомянул, что Вика любит большие числа? Нет? Ну так это чистая правда. Чем больше, тем лучше. Поэтому давайте будет писать не сколько
ей лет, а сколько дней.

Котлин поддерживает все основные математические операции: `+` умножение, `-` вычитание, `*` умножение, `/` деление. Чтобы получить из года
...
У каждой переменной есть тип. Например, число или строка. При объявлении переменной тип можно задать явно или же, если мы сразу присваиваем
значение, то Котлин сам поймет какой тип должен быть у переменной. Тип задается следующим образом:
...
По большому счету получилось та же самая функция `main` только с одним отличием - у нашей функции есть ~~железный~~ аргумент. Пристальнее
...
__Задача:__ напишите цепочку вызовов функций глубиной 3. Каждая функция при вызове должна печатать свое название и вызывать следующую
