...
Давайте напишем приложение, которое будет имитировать игральные кости. Сформулируем основные возможности нашего приложения:
...
присваивать полченное значение в наше текстовое вью.
...
на экран layout'а. Кликните на вкладке `Buttons` и перетащите на экран 1 __RadioGroup__ и 6 __RadioButton__'ов. _RadioButton_ - это элемент,
...
Если __RadioGroup__ у вас подсвечивается красным, попробуйте добавить импорт:
...
Дальше нам надо определить какой был сделан выбор из радио-кнопок при нажатии на кнопку `Бросить`:
...
* `R.id.choice1 -> choice = 1` - на каждой строчке мы сначала пишем чем должен быть равен аргумент, а затем через стрелку `->` пишем что
